...
public class CityController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<CityGetDTO> getCityById(@PathVariable Integer id){
        log.info("Received request to get city with ID: {}", id);
        return cityService.getById(id);
    }

    @PostMapping
    public ResponseEntity<CityGetDTO> createCity(@Valid @RequestBody CityCreateDTO city){
        log.info("Received request to create a new city: {}", city);
        return cityService.create(city);
    }

    @PutMapping("/{id}")
    public ResponseEntity<CityGetDTO> updateCity(@PathVariable Integer id, @Valid @RequestBody CityUpdateDTO city){
